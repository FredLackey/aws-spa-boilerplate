...
# go-d.sh
# Main orchestration script for Stage D React deployment
# Coordinates the entire React deployment workflow by calling helper scripts sequentially
...
Stage D React deployment requires successful completion of Stage A, Stage B, and Stage C.
No additional command line arguments are required - all configuration is derived
from previous stage outputs.

Examples:
  $0                    # Deploy React application using Stage A, B, and C outputs
...
Notes:
  ...
  - React application will be built and deployed to the existing S3 bucket
  - CloudFront cache will be invalidated for immediate content updates
...
if [[ $# -gt 0 ]]; then
    echo "❌ Error: Stage D does not accept command line arguments."
...
echo "🚀 AWS SPA Boilerplate - Stage D React Deployment"
echo "================================================="
echo "This script will build and deploy the React application to CloudFront."
echo "Each step builds upon the previous stages, so please ensure A, B, and C are completed."
echo
...
handle_script_error() {
    ...
    case "$script_name" in
        "gather-inputs.sh")
            echo "Input collection failed. Please check Stage A, B, and C completion, then re-run this script."
            ;;
        "aws-discovery.sh")
    ...
    echo
    echo "Options:"
    echo "1. Fix the issue and re-run: ./go-d.sh"
...
check_stage_prerequisites() {
    echo "🔍 Checking Stage A, B, and C prerequisites..."
    ...
    if [[ "$ready_for_stage_b" != "true" ]]; then
        ...
        echo "   Please ensure Stage A completed successfully before running Stage D"
        exit 1
    fi
    ...
    if [[ "$ready_for_stage_c" != "true" ]]; then
        ...
        echo "   Please ensure Stage B completed successfully before running Stage D"
        exit 1
    fi
    ...
    if [[ "$ready_for_stage_d" != "true" ]]; then
        ...
        echo "   Please ensure Stage C completed successfully before running Stage D"
        exit 1
    fi
    
    echo "✅ Stage A, B, and C prerequisites verified"
}

# Main deployment orchestration function
main_deployment() {
    echo "Starting Stage D React deployment workflow..."
    echo
    
    # Pre-check: Ensure Stage A, B, and C are completed
    check_stage_prerequisites
    echo
    
    # Step 1: Gather Inputs
    execute_step 1 "inputs" "gather-inputs.sh" "Validate Stage A/B/C completion and gather configuration"
