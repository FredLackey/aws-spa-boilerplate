...
# CDK deployment orchestration for Stage E React API deployment
# Builds React JSON application and deploys content to CloudFront distribution with API integration
...
REACT_APP_DIR="$STAGE_DIR/../../apps/hello-world-json"

echo "=== Stage E React API Deployment - Infrastructure Deployment ==="
echo "This script will build the React JSON application and deploy content to CloudFront with API integration."
echo
...
validate_prerequisites() {
    ...
    echo "React JSON application directory: $REACT_APP_DIR"
}
...
generate_cdk_context() {
    ...
    local primary_domain certificate_arn lambda_function_arn lambda_function_url
    ...
    lambda_function_arn=$(jq -r '.lambdaFunctionArn' "$inputs_file")
    ...
    jq --arg prefix "$distribution_prefix" \
       ...
       '.context."stage-e-react-api:distributionPrefix" = $prefix |
        .context."stage-e-react-api:targetRegion" = $region |
        .context."stage-e-react-api:targetProfile" = $profile |
        .context."stage-e-react-api:infrastructureProfile" = $infra_profile |
        .context."stage-e-react-api:targetAccountId" = $target_account |
        .context."stage-e-react-api:infrastructureAccountId" = $infra_account |
        .context."stage-e-react-api:targetVpcId" = $vpc_id |
        .context."stage-e-react-api:distributionId" = $dist_id |
        .context."stage-e-react-api:bucketName" = $bucket |
        .context."stage-e-react-api:primaryDomain" = $domain |
        .context."stage-e-react-api:certificateArn" = $cert_arn |
        .context."stage-e-react-api:lambdaFunctionArn" = $lambda_arn |
        .context."stage-e-react-api:lambdaFunctionUrl" = $lambda_url' \
       "$cdk_json" > "$cdk_json.tmp" && mv "$cdk_json.tmp" "$cdk_json"
    
    echo "✅ CDK context updated"
}

# Function to clean previous React build
clean_react_build() {
    ...
    # Remove node_modules if package-lock.json has changed
    if [[ -d "node_modules" ]] && [[ -f "package-lock.json" ]]; then
        if [[ "package-lock.json" -nt "node_modules" ]]; then
            echo "   Dependencies appear outdated, removing node_modules..."
            rm -rf node_modules
        fi
    fi
    ...
    cat > "$outputs_file" << EOF
{
  ...
  "reactApiApplicationDeployed": true,
...
main() {
    echo "Starting Stage E React deployment process..."
    echo
    ...
    echo "🎉 Stage E React deployment completed successfully!"
    echo
    echo "📋 Summary:"
    echo "   ✅ React application built successfully"
