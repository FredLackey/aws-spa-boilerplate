...
# gather-inputs.sh - Collect inputs for Stage E React API deployment
# This script validates Stage A, B, C, and D completion and prepares inputs for React API deployment
...
Stage E React API deployment requires successful completion of Stages A, B, C, and D.
No additional command line arguments are required - all configuration is derived
from previous stage outputs.

Examples:
  $0                    # Deploy React SPA with API integration using Stages A-D outputs
...
Notes:
  ...
  - React application with API integration will be built and deployed to CloudFront
  - All configuration is automatically derived from previous stages

EOF
}

# Parse command line arguments (minimal for Stage E)
while [[ $# -gt 0 ]]; do
    case $1 in
        -h|--help)
            show_usage
        ...
        *)
            ...
            echo "Stage E does not require additional command line arguments."
...
echo "=== Stage E React API Deployment - Input Validation ==="
echo "Validating Stage A, B, C, and D completion and gathering configuration..."
echo
...
# Function to load and validate Stage D outputs
load_stage_d_outputs() {
    local stage_d_outputs="$STAGE_D_DIR/data/outputs.json"
    
    echo "📋 Loading Stage D outputs..."
    echo "   Checking: $stage_d_outputs"
    
    if [[ ! -f "$stage_d_outputs" ]]; then
        echo "❌ Error: Stage D outputs not found at: $stage_d_outputs"
        echo "   Please complete Stage D (React) deployment before running Stage E"
        return 1
    fi
    
    # Validate Stage D completion status
    ...
    if [[ "$ready_for_stage_e" != "true" ]]; then
        echo "❌ Error: Stage D is not properly completed"
        echo "   Stage D outputs indicate: readyForStageE = $ready_for_stage_e"
        echo "   Please ensure Stage D completed successfully"
        return 1
    fi
    
    echo "✅ Stage D outputs loaded successfully"
    echo "   React deployment validated"
    echo
}

# Function to validate React JSON application exists
validate_react_json_application() {
    local react_app_dir="$STAGE_DIR/../../apps/hello-world-json"
    
    echo "📋 Validating React JSON application..."
    echo "   Checking: $react_app_dir"
    
    if [[ ! -d "$react_app_dir" ]]; then
        echo "❌ Error: React JSON application directory not found at: $react_app_dir"
        return 1
    fi
    
    if [[ ! -f "$react_app_dir/package.json" ]]; then
        echo "❌ Error: React JSON application package.json not found"
        return 1
    fi
    
    if [[ ! -f "$react_app_dir/vite.config.js" ]]; then
        echo "❌ Error: React JSON application vite.config.js not found"
        return 1
    fi
    
    if [[ ! -f "$react_app_dir/index.html" ]]; then
        echo "❌ Error: React JSON application index.html not found"
        return 1
    fi
    
    echo "✅ React JSON application validated"
